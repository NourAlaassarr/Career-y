...
const SkillQuizPage = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const data = await httpGet(`http://localhost:8000/Quiz/Quiz?SkillId=${skillId}`, {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await httpPost(`http://localhost:8000/Quiz/SubmitTopicQuiz?SkillId=${skillId}`, {
        answer: answerArray
      }, { headers: { 'token': session.token } });

      const { Grade, TotalQuestions } = response;
      setGrade(Grade);
      navigate(`/quiz/${skillId}/grade`, { state: { grade: Grade, totalQuestions: TotalQuestions } });
    } catch (error) {
  ...
  const formatTime = time => `${Math.floor(time / 60)}:${('0' + (time % 60)).slice(-2)}`;
